...
[![BRE Standard](https://img.shields.io/badge/Standard-BS%20373%3A1957-blue?style=flat-square)](https://bregroup.com)
...
## 🌟 Key Accomplishments & Audit Summary
...
- **191 Unique Common Species**: 186 original species from Lavers (1983) plus **5 brand new commercial species** introduced in the 1997 Supplement (*Andiroba*, *Bintangor*, *Kamarare*, *Taun*, *Vitex*).
- **223 Species & Location Lots**: 221 distinct `(Common Name + Botanical Name + Origin)` provenance combinations plus 2 regional sub-lots (Teak Nigeria Western vs Northern).
- **1:1 Botanical Index Verification**: Verified all 206 lines of the printed Botanical Name Index (Pages 58–59 of 1983 report) against Table 2 provenance blocks.
- **External Scientific Audit**: Validated data accuracy against independent BRE literature (*"The strength of UK-grown larch"*, Table 7), achieving 100% exact numerical alignment.
...
├── master_lavers_1983.json              # 1983 Master Dataset (435 records, 100% untouched)
...
For each species and test condition (*Green* or *Air-Dried at 12% Moisture Content*), the following properties are recorded:
...
## 🚀 How to Load & Host on GitHub Pages (Static Web Site)

`index.html` is built as a **zero-dependency static web application**. The entire dataset is bundled directly inside the file, meaning it requires **no build step, no npm, and no backend server**.

### Step 1: Push Code to Personal GitHub (`jjsolly`)

```powershell
# Change directory to the clean project folder
cd c:\Users\jjs\Documents\Code\Lavers1983\bre-timber-strength-properties

# Initialize git repository
git init
git add .
git commit -m "Initial commit: BRE Timber Strength Properties Dataset & Explorer"

# Connect remote to lavers-timber-strength-properties
git remote add origin https://github.com/jjsolly/lavers-timber-strength-properties.git
git branch -M main
git push -u origin main
```

### Step 2: Enable GitHub Pages on Personal Repo
1. Go to `https://github.com/jjsolly/lavers-timber-strength-properties/settings/pages`.
2. Under **Build and deployment**:
   - **Source**: Select `Deploy from a branch`.
   - **Branch**: Select `main` branch, directory `/ (root)`.
3. Click **Save**.

Your personal static website will be live at:  
👉 `https://jjsolly.github.io/lavers-timber-strength-properties/`

### Step 3: Fork to Organization (`formateng`)
1. Go to `https://github.com/jjsolly/lavers-timber-strength-properties`.
2. Click **Fork** (top right) $\rightarrow$ Select **Owner**: `formateng` $\rightarrow$ Click **Create fork**.
3. Enable Pages on the fork: `https://github.com/formateng/lavers-timber-strength-properties/settings/pages`.
4. Organization live website:  
👉 `https://formateng.github.io/lavers-timber-strength-properties/`
